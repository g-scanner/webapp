...
void main() {
  ...
  group('GROUP 1 – Header & section titles', () {
    testWidgets('renders page title, subtitle and all section headers',
      ...
      expect(find.text('settings.sectionTitles.dataAndHistory'), findsOneWidget);
  ...
  group('GROUP 2 – Account card presentation', () {
    ...
    testWidgets(
        'registered user with empty displayName renders fallback key',
        (tester) async {
      when(() => user.isAnonymous).thenReturn(false);
  ...
  group('GROUP 3 – Analysis rule toggles', () {
    testWidgets('warnAdditives switch calls onSettingsChange(warnAdditives:true)',
        (tester) async {
      await _pump(tester, cb: cb, auth: auth, settings: _settings(warnAdditives: false));
    ...
    testWidgets('strictMode switch calls onSettingsChange(strictMode:true)',
        (tester) async {
      await _pump(tester, cb: cb, auth: auth, settings: _settings(strictMode: false));
    ...
    testWidgets('alertLactose switch calls onSettingsChange(alertLactose:true)',
        (tester) async {
      await _pump(tester, cb: cb, auth: auth, settings: _settings(alertLactose: false));
    ...
    testWidgets('section toggle titles are visible', (tester) async {
      ...
      expect(find.text('settings.toggles.lactoseIntoleranceTitle'), findsOneWidget);
    });
  });
  ...
  group('GROUP 4 – Theme selector', () {
    ...
    testWidgets('selecting dark theme calls onSettingsChange(preferredTheme:dark)',
        (tester) async {
      await _pump(
        tester,
        cb: cb,
      ...
      verify(() => cb.onSettingsChange(
            any(
              that: isA<UserSettings>()
                  .having((s) => s.preferredTheme, 'preferredTheme', 'dark'),
            ),
          )).called(1);
    });

    testWidgets('selecting light theme calls onSettingsChange(preferredTheme:light)',
        (tester) async {
      await _pump(
        tester,
        cb: cb,
      ...
      verify(() => cb.onSettingsChange(
            any(
              that: isA<UserSettings>()
                  .having((s) => s.preferredTheme, 'preferredTheme', 'light'),
            ),
          )).called(1);
    });

    testWidgets('selecting already-active theme does NOT call onSettingsChange',
        (tester) async {
      await _pump(
        tester,
        cb: cb,
  ...
  group('GROUP 5 – Language selector', () {
    testWidgets('selecting a different language calls onSettingsChange',
      ...
      verify(() => cb.onSettingsChange(
            any(
              that: isA<UserSettings>()
                  .having((s) => s.preferredLanguage, 'preferredLanguage', 'en'),
            ),
          )).called(1);
    });

    testWidgets('selecting already-active language does NOT call onSettingsChange',
        (tester) async {
      await _pump(
        tester,
        cb: cb,
  ...
  group('GROUP 6 – Clear History flow', () {
    testWidgets('tapping clear history shows confirmation dialog',
      ...
      await tester.tap(find.text('settings.destructive.clearHistoryTitle').first);
      await tester.pumpAndSettle();
    ...
    testWidgets('cancelling dialog does NOT call onClearHistory', (tester) async {
      await _pump(tester, cb: cb, auth: auth);
  ...
  group('GROUP 7 – Legal information section', () {
    testWidgets('all three legal items are rendered', (tester) async {
      await _pump(tester, cb: cb, auth: auth);
      expect(find.text('settings.legalMenu.termsAndConditionsTitle'), findsOneWidget);
      expect(find.text('settings.legalMenu.privacyPolicyTitle'), findsOneWidget);
  ...
  group('GROUP 8 – Anonymous User Conversion Flow', () {
    ...
    testWidgets('cancelling conversion dialog does NOT delete user or sign out',
        (tester) async {
      when(() => user.isAnonymous).thenReturn(true);
      when(() => auth.currentUser).thenReturn(user);
  ...
  group('GROUP 9 – Profile Bottom Sheet Presentation', () {
    testWidgets('opens bottom sheet with profile info and Google provider data',
        (tester) async {
    ...
    testWidgets('renders fallback "addYourName" when displayName is empty in bottom sheet',
        (tester) async {
      when(() => user.isAnonymous).thenReturn(false);
      when(() => user.displayName).thenReturn('');
      when(() => user.email).thenReturn('mario@example.com');
  ...
  group('GROUP 10 – Edit Name Flow with Optimistic Update', () {
    ...
    testWidgets('back button in edit name view returns to menu view without updating',
        (tester) async {
      when(() => user.isAnonymous).thenReturn(false);
      when(() => user.displayName).thenReturn('Mario Rossi');
      when(() => auth.currentUser).thenReturn(user);
    ...
    testWidgets('saving new name triggers Optimistic Update and calls user.updateDisplayName',
        (tester) async {
      when(() => user.isAnonymous).thenReturn(false);
      when(() => user.displayName).thenReturn('Mario Rossi');
      when(() => auth.currentUser).thenReturn(user);
  ...
  group('GROUP 12 – Account Deletion Flow', () {
    ...
    testWidgets(
        'Scenario A: Cancelling re-auth dialog does NOT sign out',
        (tester) async {
      when(() => metadata.lastSignInTime).thenReturn(
        DateTime.now().subtract(const Duration(minutes: 10)),
      );
    ...
    testWidgets(
      ...
      expect(find.text('settings.account.deleteConfirmTitle'), findsOneWidget);
