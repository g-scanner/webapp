...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockCallbacks.onReportSubmit(any(), any()),
    ).thenAnswer((_) async {});
    when(() => mockCallbacks.onProductUpdate(any())).thenAnswer((_) async {});
    when(
      () => mockCallbacks.onDeleteHistoryByBarcode(any()),
    ).thenAnswer((_) async {});
  ...
  Product createSampleProduct({
    ...
    return Product(
      barcode: barcode,
      nameMap:
          nameMap ?? {'it': 'Pasta Senza Glutine', 'en': 'Gluten Free Pasta'},
      ...
      imageUrl: imageUrl,
      lastUpdated: lastUpdated ?? DateTime.now().subtract(const Duration(days: 5)).toIso8601String(),
    );
  }
    ...
    await tester.pumpWidget(
      createTestApp(
        child: ProductDetailCard(
          ...
          onDeleteHistoryByBarcode: passDeleteHistoryCallback
              ? mockCallbacks.onDeleteHistoryByBarcode
              : null,
          hasReportedThisSession: hasReportedThisSession,
          userReportId: userReportId,
          onDeleteReport: passDeleteReportCallback
              ? mockCallbacks.onDeleteReport
              : null,
          showReportLink: showReportLink,
          showScanDate: showScanDate,
          scannedAt: scannedAt,
          onViewReport: passViewReportCallback
              ? mockCallbacks.onViewReport
              : null,
  ...
  group('ProductDetailCard Widget & Business Logic Tests', () {
    ...
    // ==========================================
    testWidgets(
      'Header renders title and back button triggers onBack callback',
    ...
    testWidgets('3-Dots menu: delete history dialog flow (cancel vs confirm)', (
      ...
      final deleteHistoryItem = find.text(
        'common.actions.deleteHistoryConfirmTitle',
      );
      ...
      verify(
        () => mockCallbacks.onDeleteHistoryByBarcode('8009999999999'),
      ).called(1);
      verify(() => mockCallbacks.onBack()).called(1);
    });

    testWidgets(
      '3-Dots menu: delete report dialog flow triggers onDeleteReport',
      (WidgetTester tester) async {
        final product = createSampleProduct();
        ...
        final deleteReportItem = find.text(
          'common.actions.deleteReportConfirmTitle',
        );
    ...
    testWidgets(
      '3-Dots menu: renders and opens when isInHistoryNotifier is null but onDeleteHistoryByBarcode is provided',
      (WidgetTester tester) async {
    ...
    // ==========================================
    testWidgets(
      'Status Safe: displays safe status banner for gluten-free product',
      (WidgetTester tester) async {
    ...
    testWidgets(
      'Status Unsafe: displays unsafe status banner for product with gluten ingredients',
      (WidgetTester tester) async {
    ...
    testWidgets(
      'Status Uncertain: reports elevate status to uncertain with warning banner',
      (WidgetTester tester) async {
        final reportedProduct = createSampleProduct(pendingReportsCount: 2);
    ...
    // ==========================================
    testWidgets(
      'Lactose alert: displayed when alertLactose is enabled and product has lactose',
      (WidgetTester tester) async {
    ...
    // ==========================================
    testWidgets(
      'Allergens: shows insufficientDataLabel for ghost/incomplete products',
      (WidgetTester tester) async {
    ...
    testWidgets(
      'Allergens: shows noneLabel when ingredients exist with no declared allergens',
      (WidgetTester tester) async {
    ...
    testWidgets(
      'Allergens: does NOT render chip for "Senza Glutine" or "en:gluten-free" and shows noneLabel when ingredients exist',
      (WidgetTester tester) async {
    ...
    testWidgets(
      'Allergens: does NOT render chip for "Senza Glutine" but renders real allergens alongside it',
      (WidgetTester tester) async {
    ...
    // ==========================================
    testWidgets(
      'Reactivity: productNotifier updates product details in place',
      (WidgetTester tester) async {
    ...
    // ==========================================
    testWidgets(
      'View Report: navigates to report details when tapping report banner link',
      (WidgetTester tester) async {
    ...
    // ==========================================
    // 9. STALE DATA WARNING CARD (FOOD SAFETY)
    // ==========================================
    testWidgets(
      'Stale Data: does NOT show warning card when product is fresh',
      (WidgetTester tester) async {
        final product = createSampleProduct();
    ...
    testWidgets('Stale Data: renders warning card when isStaleData is true', (
      WidgetTester tester,
    ) async {
      final product = createSampleProduct();
    ...
    testWidgets(
      'Stale Data: reactively displays card when isStaleDataNotifier emits true',
      (WidgetTester tester) async {
        final product = createSampleProduct();
        final staleNotifier = ValueNotifier<bool>(false);
