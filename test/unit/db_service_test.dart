...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockDb.collection(productsCollection)).thenReturn(mockProductsCol);
  ...
  group('GROUP 1 – Local Product Cache', () {
    ...
    test('saveLocalProducts and getLocalProducts persists and retrieves products', () async {
    ...
    test('getLocalProductByBarcode returns product if found or null if absent', () async {
    ...
    test('upsertLocalProduct updates existing item or prepends new item', () async {
  ...
  group('GROUP 2 – Firestore Delta Sync & Product Lookup', () {
    test('performDeltaSync on first launch queries orderBy last_updated limit 100', () async {
      ...
      when(() => mockProductsCol.orderBy('last_updated', descending: true)).thenReturn(mockQuery);
    ...
    test('performDeltaSync incremental queries where last_updated is greater than lastSync', () async {
      await DbService.saveLastSyncTime('2026-08-01T00:00:00Z');
      await DbService.saveLocalProducts([createSampleProduct(barcode: '100', name: 'Existing')]);

      final mockQuery = MockQuery();
      final mockSnap = MockQuerySnapshot();
      final mockDoc = MockQueryDocumentSnapshot();

      final updatedProd = createSampleProduct(barcode: '200', name: 'New Remote');
      when(() => mockDoc.data()).thenReturn(updatedProd.toJson());
      when(() => mockSnap.docs).thenReturn([mockDoc]);

      when(() => mockProductsCol.where('last_updated', isGreaterThan: '2026-08-01T00:00:00Z'))
    ...
    test('getProductByBarcode fetches from Firestore, caches locally, and returns product', () async {
  ...
  group('GROUP 3 – Scanning Pipeline (scanBarcodeClientSide)', () {
    ...
    test('fetches from Firestore if absent locally and populates cache', () async {
      ...
      final result = await DbService.scanBarcodeClientSide('remote_1', settings);
    ...
    test('fetches from Open Food Facts when missing locally and parses dirty ingredients with regex', () async {
      ...
      expect(await DbService.getLocalProductByBarcode('off_prod_1'), isNotNull);
    });

    test('OFF parsing fallback to generic languages and allergens_from_ingredients', () async {
      ...
      final rawOffJson = {
        ...
        "product": {
          ...
          "ingredients_text_pl": "Maka ryzowa, cukier", // Non standard language
    ...
    test('triggers silent background OFF refresh when local product fetchedFromOffAt >= 30 days', () async {
      final oldDate = DateTime.now().subtract(const Duration(days: 40)).toIso8601String();
      ...
      await http.runWithClient(() async {
        ...
        final result = await DbService.scanBarcodeClientSide('stale_prod_1', settings);
      ...
      verify(() => mockDocRef.set(any(), any())).called(1);
      final updatedLocal = await DbService.getLocalProductByBarcode('stale_prod_1');
  ...
  group('GROUP 4 – History Management', () {
    ...
    test('suppresses duplicate scan history entries within 10 seconds for same barcode', () async {
      final settings = UserSettings(
        strictMode: true,
        alertLactose: false,
        warnAdditives: true,
    ...
    test('deleteHistoryByBarcodeLocal removes all occurrences of barcode', () async {
      final prefs = await SharedPreferences.getInstance();
      final items = [
        ScanHistoryItem(id: '1', barcode: 'TARGET', scannedAt: '2026-01-01T00:00:00Z'),
        ScanHistoryItem(id: '2', barcode: 'OTHER', scannedAt: '2026-01-02T00:00:00Z'),
        ScanHistoryItem(id: '3', barcode: 'TARGET', scannedAt: '2026-01-03T00:00:00Z'),
      ];
      await prefs.setStringList(
        'celiac_history',
        items.map((e) => json.encode(e.toJson())).toList(),
    ...
    test('deleteHistoryItemLocal removes specific item by id', () async {
      final prefs = await SharedPreferences.getInstance();
      final items = [
        ScanHistoryItem(id: 'item_1', barcode: '111', scannedAt: '2026-01-01T00:00:00Z'),
        ScanHistoryItem(id: 'item_2', barcode: '222', scannedAt: '2026-01-02T00:00:00Z'),
      ];
      await prefs.setStringList(
        'celiac_history',
        items.map((e) => json.encode(e.toJson())).toList(),
    ...
    test('wipeHistoryLocal clears all items and batch deletes Firestore docs when logged in', () async {
      ...
      when(() => mockDb.collection('users/user_123/history')).thenReturn(mockUserHistoryCol);
      when(() => mockUserHistoryCol.get()).thenAnswer((_) async => mockHistSnap);
  ...
  group('GROUP 5 – Product Reports & Voting', () {
    test('submitProductReportClientSide performs atomic batch write and persists local report', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      when(() => mockUser.isAnonymous).thenReturn(false);
    ...
    test('deleteLocalReport removes report and removes barcode from reported list', () async {
      final prefs = await SharedPreferences.getInstance();
      ...
      await prefs.setStringList('celiac_reports', [json.encode(report.toJson())]);
      await prefs.setStringList('celiac_reported_barcodes', ['BARCODE_9', 'BARCODE_OTHER']);
      ...
      final reportedBarcodes = prefs.getStringList('celiac_reported_barcodes') ?? [];
      expect(reportedBarcodes, ['BARCODE_OTHER']);
    });

    test('deleteReportFromDb performs atomic batch delete of report, votes, decrements pending count, and removes from user', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      when(() => mockUser.isAnonymous).thenReturn(false);
      ...
      when(() => mockReportsCol.doc('rep_del_123')).thenReturn(mockReportDocRef);
      when(() => mockReportDocRef.get()).thenAnswer((_) async => mockReportSnap);
      when(() => mockReportSnap.exists).thenReturn(true);
      when(() => mockReportSnap.data()).thenReturn({'barcode': '800BARCODE_DEL'});
      ...
      when(() => mockProductsCol.doc('800BARCODE_DEL')).thenReturn(mockDocRef);
    ...
    test('deleteReportFromDb returns early when report does not exist', () async {
      final mockReportDocRef = MockDocumentReference();
      final mockReportSnap = MockDocumentSnapshot();

      when(() => mockReportsCol.doc('rep_missing')).thenReturn(mockReportDocRef);
      when(() => mockReportDocRef.get()).thenAnswer((_) async => mockReportSnap);
  ...
  group('GROUP 6 – User Settings', () {
    test('getLocalSettings returns default settings when no saved config exists', () async {
    ...
    test('saveSettings writes locally and syncs to Firestore if user is authenticated', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      when(() => mockUser.isAnonymous).thenReturn(false);
  ...
  group('GROUP 7 – Anonymous Data Migration & Wipe', () {
    test('migrateLocalDataToFirestore migrates history, reports, and reported barcodes', () async {
      final prefs = await SharedPreferences.getInstance();

      final fakeHistory = [
        ScanHistoryItem(id: 'h1', barcode: '111', scannedAt: '2026-08-01T00:00:00Z'),
      ];
      final fakeReports = [
        ProductReport(
          id: 'r1',
      ...
      await prefs.setStringList('celiac_history', [json.encode(fakeHistory.first.toJson())]);
      await prefs.setStringList('celiac_reports', [json.encode(fakeReports.first.toJson())]);
      await prefs.setStringList('celiac_reported_barcodes', ['222']);

      final mockUserHistoryCol = MockCollectionReference();
      when(() => mockDb.collection('users/new_user_123/history')).thenReturn(mockUserHistoryCol);
      ...
      verify(() => mockBatch.commit()).called(2); // 1 for history, 1 for reports
    });
  ...
  group('GROUP 8 – Terms & Account Deletion Helpers', () {
    test('hasAcceptedTerms and saveTermsAccepted manages terms agreement state', () async {
    ...
    test('deleteUserHistory deletes all docs in user history subcollection', () async {
      final mockUserHistoryCol = MockCollectionReference();
      final mockSnap = MockQuerySnapshot();
      final mockDoc = MockQueryDocumentSnapshot();

      when(() => mockUsersCol.doc('user_hist_del')).thenReturn(mockDocRef);
      when(() => mockDocRef.collection('history')).thenReturn(mockUserHistoryCol);
    ...
    test('anonymizeUserReports updates reports to userId: deleted and anonymized: true', () async {
      final mockQuery = MockQuery();
      final mockSnap = MockQuerySnapshot();
      final mockDoc = MockQueryDocumentSnapshot();

      when(() => mockReportsCol.where('userId', isEqualTo: 'user_to_delete'))
          .thenReturn(mockQuery);
      when(() => mockQuery.get()).thenAnswer((_) async => mockSnap);
      when(() => mockSnap.docs).thenReturn([mockDoc]);
      when(() => mockDoc.reference).thenReturn(mockDocRef);
      ...
      verify(() => mockBatch.update(mockDocRef, {
        'userId': 'deleted',
        'anonymized': true,
      })).called(1);
      verify(() => mockBatch.commit()).called(1);
  ...
  group('GROUP 9 – Voting, Report Sync & Cloud Sync Edge Cases', () {
    test('syncHistoryWithFirestore fetches remote history for authenticated user', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      when(() => mockUser.isAnonymous).thenReturn(false);
      ...
      final remoteItem = ScanHistoryItem(id: 'rem_1', barcode: '888', scannedAt: '2026-08-01T00:00:00Z');
      when(() => mockDoc.data()).thenReturn(remoteItem.toJson());
      when(() => mockSnap.docs).thenReturn([mockDoc]);

      when(() => mockDb.collection('users/sync_user_1/history')).thenReturn(mockUserHistoryCol);
      when(() => mockUserHistoryCol.orderBy('scannedAt', descending: true)).thenReturn(mockQuery);
      when(() => mockQuery.limit(100)).thenReturn(mockQuery);
      when(() => mockQuery.get()).thenAnswer((_) async => mockSnap);
    ...
    test('syncReportsWithFirestore fetches remote reports for authenticated user and sorts descending', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      when(() => mockUser.isAnonymous).thenReturn(false);
      ...
      when(() => mockReportsCol.where('userId', isEqualTo: 'sync_reporter')).thenReturn(mockQuery);
      when(() => mockQuery.get()).thenAnswer((_) async => mockSnap);
    ...
    test('voteOnReportByBarcode creates vote and updates report score via batch', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      ...
      when(() => mockReportsCol.where('barcode', isEqualTo: '800VOTE')).thenReturn(mockQuery);
      when(() => mockQuery.where('status', isEqualTo: 'open')).thenReturn(mockQuery);
    ...
    test('getReportVoteDataByBarcode retrieves report score and user vote', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      when(() => mockUser.uid).thenReturn('voter_1');
    ...
    test('syncSettingsWithFirestore merges remote and local reported barcodes', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      when(() => mockUser.isAnonymous).thenReturn(false);
      ...
      expect(merged.reportedBarcodes, containsAll(['LOCAL_1', 'REMOTE_1', 'SHARED_BARCODE']));
    });
  });
  ...
  group('GROUP 10 – Anonymous Early Returns, Error Handling & Unsynced Wrappers', () {
    test('scanBarcodeClientSide with autoSaveHistory: false does not record history', () async {
      ...
      await DbService.scanBarcodeClientSide('NO_HIST_PROD', noHistorySettings);

      final history = await DbService.getHistory();
      expect(history, isEmpty);
    });

    test('getLocalUnsyncedHistory and getLocalUnsyncedReports read anonymous storage keys', () async {
      final prefs = await SharedPreferences.getInstance();
      final fakeHistory = [ScanHistoryItem(id: 'u_h1', barcode: '888', scannedAt: '2026-08-01T00:00:00Z')];
      final fakeReports = [
        ProductReport(
          id: 'u_r1',
          barcode: '999',
    ...
    test('anonymous users (isAnonymous == true or user == null) return local data immediately without Firestore calls', () async {
      // 1. User is anonymous
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      ...
      final settingsResult = await DbService.syncSettingsWithFirestore(localSettings);
      ...
      verifyNever(() => mockReportsCol.where('userId', isEqualTo: any(named: 'isEqualTo')));
    });

    test('wipeCurrentUserLocalData delegates cleanly to wipeAllLocalData', () async {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setStringList('celiac_history', ['{}']);
      await prefs.setString('celiac_settings', '{}');
    ...
    test('handles Firestore exceptions in deltaSync, getProductByBarcode, and sync methods without crashing', () async {
      when(() => mockAuth.currentUser).thenReturn(mockUser);
      when(() => mockUser.isAnonymous).thenReturn(false);
      when(() => mockUser.uid).thenReturn('err_user');

      // 1. getProductByBarcode exception -> returns null
      when(() => mockProductsCol.doc('ERR_DOC')).thenThrow(Exception('Firestore offline'));
      ...
      when(() => mockProductsCol.orderBy('last_updated', descending: true))
      ...
      when(() => mockDb.collection('users/err_user/history')).thenThrow(Exception('Quota exceeded'));
      ...
      when(() => mockReportsCol.where('userId', isEqualTo: 'err_user'))
      ...
      when(() => mockUsersCol.doc('err_user')).thenThrow(Exception('Server error'));
