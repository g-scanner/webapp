...
void main() {
  ...
  group('ModularAssetLoader Unit Tests', () {
    ...
    test(
      'loads and merges all Italian modular locale JSON files into a single map',
      () async {
        ...
        for (final module in expectedModules) {
          expect(
            result.containsKey(module),
            isTrue,
            reason: 'Missing module namespace: $module',
          );
          expect(
            result[module],
            isA<Map<String, dynamic>>(),
            reason: 'Module $module is not a Map',
          );
        }
      },
    );
    ...
    test(
      'retrieves nested translation keys correctly from merged map',
      () async {
        final result = await loader.load('assets/locales', const Locale('it'));

        final authMap = result['auth'] as Map<String, dynamic>;
        expect(authMap['social'], isNotNull);
        expect(authMap['legal'], isA<Map<String, dynamic>>());
    ...
    test(
      'returns empty map when locale has no matching json asset files (e.g. en, xx)',
      () async {
        final resultEn = await loader.load(
          'assets/locales',
          const Locale('en'),
        );
        expect(resultEn, isEmpty);

        final resultXx = await loader.load(
          'assets/locales',
          const Locale('xx'),
        );
